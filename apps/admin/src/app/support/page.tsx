...
export default function AMARADashboard() {
  ...
  return (
    <div>
      ...
      {/* Recent Playbook Updates */}
      {recentPlaybooks.length > 0 && (
        <div className="bg-gray-800 rounded-xl border border-gray-700 p-5 mb-6">
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-semibold text-sm">📋 Recent Playbook Updates</h2>
            <a href="/support/playbooks" className="text-xs text-gray-500 hover:text-gray-300">View all →</a>
          </div>
          <div className="space-y-2">
            {recentPlaybooks.map((p: any) => (
              <div key={p.id} className="flex items-center justify-between py-2 border-b border-gray-700 last:border-0">
                <div className="flex items-center gap-3">
                  <span className="font-mono text-xs text-emerald-400">{p.issueId}</span>
                  <span className="text-xs text-gray-400 capitalize">{p.feature}</span>
                  {p.autoCreated && <span className="text-[10px] bg-blue-600/20 text-blue-400 px-1.5 py-0.5 rounded">Auto-created by AMARA</span>}
                </div>
                <span className="text-[10px] text-gray-600">{new Date(p.lastUpdated).toLocaleDateString()}</span>
              </div>
            ))}
          </div>
        </div>
